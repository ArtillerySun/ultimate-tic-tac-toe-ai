...
def extract_features(state: State):
    ...
    for i in range(3):
        for j in range(3):
            ...
            if local_status[i, j] != 0 and local_status[i, j] != 3:
    ...
    x_features = np.concatenate([
        ...
        [prev_action_influence * turn_number],
        [num_of_actions],
        [num_to_win],
        [oppo_local_pairs],
        [turn_number],
...
model = UTTTNet(input_size=30).to(device)
...
torch.save(model.state_dict(), 'model/uttt_model_final.pth')
...
# svr = SVR(kernel='rbf', C=20, gamma='scale', cache_size=500)  # C=10 is arbitrary; tune it later
# svr.fit(x_train, y_train)
# train_score = svr.score(x_train, y_train)
# test_score = svr.score(x_test, y_test)
# print(f"Train Score: {train_score:.4f}, Test Score: {test_score:.4f}")
# with open("model/svr_model2.pkl", "wb") as f:
